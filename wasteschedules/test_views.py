...
# -> Credit for django test assertions: https://docs.djangoproject.com/en/5.0/topics/testing/tools/#assertions  # noqa
...
class ScheduleListTestCase(TestCase):
    ...
    def test_render_schedule_list_existing_postcode(self):
        ...
        response = self.client.get(
            reverse("schedule_list", kwargs={"postcode": "12345"})
        )
    ...
    def test_render_schedule_list_non_existing_postcode(self):
        ...
        """
        response = self.client.get(
            reverse("schedule_list", kwargs={"postcode": "11111"})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_render_schedule_list_invalid_postcode(self):
        ...
        """
        response = self.client.get(
            reverse("schedule_list", kwargs={"postcode": "11"})
        )
        self.assertEqual(response.status_code, 200)
        self.assertFalse(response.context['schedule_list'].exists())
    ...
    def test_render_schedule_list_0_postcode(self):
        ...
        This should be the case because the 0 postcode is added to the url by
        the "Browse Locations" link in the Navigation bar and should display
        all schedules.
        """
        response = self.client.get(
            reverse('schedule_list', kwargs={'postcode': '0'}))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['schedule_list'].exists())

    def test_like_context_variable(self):
        """
        Tests if the likes are being annotated to the schedule_list queryset
        correctly.
        """
        response = self.client.get(
            reverse('schedule_list', kwargs={'postcode': '12345'}))
    ...
    def test_user_likes(self):
        ...
        response = self.client.get(
            reverse('schedule_list', kwargs={'postcode': '12345'}))
        self.assertEqual(response.status_code, 200)
        # -> Credit for converting a QuerySet to a list: https://stackoverflow.com/questions/4424435/how-to-convert-a-django-queryset-to-a-list  # noqa
        self.assertEqual(
            list(response.context["user_likes"]), [self.schedule1.id])

    def test_user_subscriptions(self):
        """
        Tests if the user_subscriptions context variable is being passed
        correctly.
        """
        self.client.login(username='testuser', password='12345')
        response = self.client.get(
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            list(response.context['user_subscriptions']), [self.schedule2.id])
...
class ScheduleDetailTestCase(TestCase):
    ...
    def test_render_schedule_detail(self):
        ...
        response = self.client.get(
            reverse(
                "schedule_detail",
                kwargs={"postcode": "12345", "slug": self.schedule.slug},
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "wasteschedules/schedule_detail.html")
    ...
    def test_schedule_detail_context_variables(self):
        """
        This method tests the context variables returned by the ScheduleDetail
        view.
        It checks if the correct variables are set in the response context and
        if their values are as expected.
        """
...
class ScheduleCommentTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.schedule.locations.add(
            PostalCode.objects.create(postal_code='12345'))

    def test_comment_submission_while_logged_out(self):
        """
        This test verifies that when a user is logged out and tries to submit a
        comment,
        ...
        response = self.client.post(
            reverse(
                "schedule_comment",
                kwargs={"slug": self.schedule.slug}), post_data
        )
    ...
    def test_successful_comment_submission(self):
        ...
        response = self.client.post(
            reverse("schedule_comment", kwargs={"slug": self.schedule.slug}),
            post_data
        )
        self.assertEqual(response.status_code, 302)
...
class ScheduleCommentEditTestCase(TestCase):
    ...
    def test_schedule_comment_edit(self):
        ...
        response = self.client.post(
            reverse(
                "schedule_comment_update",
                kwargs={"pk": self.comment.pk, "slug": self.schedule.slug},
            ),
            {"body": new_comment_body},
        )
        self.assertEqual(response.status_code, 302)
        # -> Credit for refreshing the object from the database: https://stackoverflow.com/questions/4377861/reload-django-object-from-database  # noqa
...
class ScheduleCommentDeleteTestCase(TestCase):
    ...
    def test_delete_comment(self):
        ...
        self.client.login(username='testuser', password='12345')
        response = self.client.post(
            reverse("schedule_comment_delete", kwargs={"pk": self.comment.pk})
        )
        self.assertEqual(response.status_code, 302)
...
class ScheduleLikeTestCase(TestCase):
    ...
    def test_like_schedule(self):
        ...
        self.client.login(username='testuser', password='12345')
        response = self.client.post(
            reverse("schedule_like", kwargs={"slug": self.schedule.slug})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            Like.objects.filter(
                schedule_id=self.schedule.id, liked_by=self.user
            ).exists()
        )
...
class ScheduleSubscribeTestCase(TestCase):
    """
    Test case for the ScheduleSubscribe view.
    """
    ...
    def test_subscribe_to_schedule(self):
        """
        Tests if a user can subscribe to a schedule.
        """
        self.client.login(username='testuser', password='12345')
        response = self.client.post(
            reverse("schedule_subscribe", kwargs={"slug": self.schedule.slug})
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            Subscription.objects.filter(
                schedule_id=self.schedule.id, subscribed_by=self.user
            ).exists()
        )

    def test_unsubscribe_from_schedule(self):
        """
        Tests if a user can unsubscribe from a schedule.
        """
        Subscription.objects.create(
            schedule_id=self.schedule, subscribed_by=self.user)
        self.client.login(username='testuser', password='12345')
        response = self.client.post(
            reverse("schedule_subscribe", kwargs={"slug": self.schedule.slug})
        )
    ...
    def test_subscribe_to_schedule_unauthenticated(self):
        """
        Tests if an unauthenticated user can subscribe to a schedule.
        """
        response = self.client.post(
            reverse("schedule_subscribe", kwargs={"slug": self.schedule.slug})
        )
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            Subscription.objects.filter(schedule_id=self.schedule.id).exists()
        )

    def test_unsubscribe_from_schedule_unauthenticated(self):
        """
        Tests if an unauthenticated user can unsubscribe from a schedule.
        """
        Subscription.objects.create(
            schedule_id=self.schedule, subscribed_by=self.user)
        response = self.client.post(
...
class DashboardTestCase(TestCase):
    ...
    def test_dashboard_context_variables(self):
        """
        Tests if the context variables are passed correctly to the dashboard
        page.
        """
        self.client.login(username='testuser', password='12345')
        response = self.client.get(reverse('dashboard'))
        ...
        self.assertEqual(
            list(response.context["subscribed_schedules"]), [self.schedule2]
        )
        # -> Credit for converting lists with different orders to sets to compare them: https://www.digitalocean.com/community/tutorials/how-to-compare-two-lists-in-python  # noqa
        self.assertEqual(
            set(response.context["owned_schedules"]),
            set([self.schedule1, self.schedule2]),
        )
    ...
    def test_dashboard_unauthenticated_user(self):
        ...
        self.assertRedirects(
            response, "/accounts/login/?next=/wasteschedules/dashboard/"
        )
