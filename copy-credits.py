    ...
    for file in files:
        ...
        if ext in comment_symbols:
            ...
            for i in range(len(lines)):
                ...
                if lines[i].strip().startswith(comment_symbols[ext] + ' -> Credit'):
                    # Add the file name, line number and remove the "# -> Credit"
                    # Split the credit line into description and URL
                    credit_parts = lines[i].strip().replace(' -> Credits for ', '').replace(' -> Credits ', '').replace(' -> Credit for ', '').replace(
                        ' -> Credit ', '').replace('-->', '').replace('*/', '').strip()[len(comment_symbols[ext]):].split(': ')
...
credits_index = next(i for i, line in enumerate(readme_lines) if '<!-- CREDITS_START -->' in line)  # noqa

# Find the index of the line with the "<!-- CREDITS_END -->" comment
acknowledgements_index = next(i for i, line in enumerate(readme_lines) if '<!-- CREDITS_END -->' in line)  # noqa
